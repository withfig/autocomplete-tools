...
function resolveAndUpdateNodePath(
  ...
  if (!statement) throw Errors.MissingSpecVariableDeclaration("newfile.ts");
  ...
  throw Errors.MissingSpecVariableDeclaration("newfile.ts");
}
...
export function merge(
  ...
  const [newSourceFile] = setupFile("newfile.ts", newFileContent, false);
  ...
  if (Node.isIdentifier(exportDeclarationExpression)) {
    ...
    throw Errors.NoDirectDefaultExport("oldfile.ts");
  } else {
    throw Errors.DefaultExportShouldBeAnIdentifier("oldfile.ts");
  }
